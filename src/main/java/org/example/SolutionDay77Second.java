...
//    to index 2 because gcd(nums[0], nums[2]) = gcd(2, 6) = 2 > 1, and then move from index 2 to index 1 because
...
public class SolutionDay77Second {
    public static boolean canTraverseAllPairs(int[] nums) {
        ...
        int[] primes4Numbers = new int[max + 1];
        for (int i = 2; i <= max; i++)
            if (primes4Numbers[i] == 0)
        ...
        for (int num : nums) {
            int val = num;
            while (val > 1) {
                int prime = primes4Numbers[val];
                uf.unite(num, prime);
                while (val % prime == 0) val = val / prime;
            }
        }

        int root = uf.find(nums[0]);
        for (int num : nums)
            if (uf.find(num) != root) return false;
        return true;
    }
}
