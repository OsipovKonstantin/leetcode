...
public class MyQueue {
    ...
    private Stack<Integer> stackForPeekAndPop;

    public MyQueue() {
        stackForPush = new Stack();
        stackForPeekAndPop = new Stack();
    }

    public void push(int x) {
        if (stackForPush.isEmpty()) {
            while (!stackForPeekAndPop.isEmpty()) {
                stackForPush.push(stackForPeekAndPop.pop());
            }
        }
        stackForPush.push(x);
    }
    ...
    public int pop() {
        if (stackForPeekAndPop.isEmpty()) {
            while (!stackForPush.isEmpty()) {
                stackForPeekAndPop.push(stackForPush.pop());
            }
        }
        return stackForPeekAndPop.pop();
    }
    ...
    public int peek() {
        ...
        return stackForPeekAndPop.peek();
    }

    public boolean empty() {
        return stackForPeekAndPop.isEmpty() && stackForPush.isEmpty();
    }
}
