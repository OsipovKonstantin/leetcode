...
//    hard. #Logical. With 1 pig we can check steps+1 buckets (if last bucket is poisoned). With 2 pigs - (steps+1)^2
//    buckets etc. For example first pig traverse rows (from 1 to step+1 rows), second pig traverse columns
//    (from 1 to step+1), so each pig choose step+1 buckets each time. If after all steps pigs are alive bucket in
//    [step+1][step+1] is poisoned
...
//    will take no time. Each pig can feed from any number of buckets, and each bucket can be fed from by any number
//    of pigs.
...
public class SolutionDay30Second {
    public static int poorPigs(int buckets, int minutesToDie, int minutesToTest) {
        int steps = minutesToTest / minutesToDie;
        int ans = 0;
        while (Math.pow((steps + 1), ans) < buckets) {
            ans++;
        }
        return ans;
    }
