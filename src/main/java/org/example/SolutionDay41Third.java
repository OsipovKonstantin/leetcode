...
//    one cup of excess liquid. The glass indexed as (1, 0) and the glass indexed as (1, 1) will share the excess
//    liquid equally, and each will get half cup of champange.
//    Example 3:
//    Input: poured = 100000009, query_row = 33, query_glass = 17
//    Output: 1.00000
public class SolutionDay41Third {
    public static double champagneTower(int poured, int query_row, int query_glass) {
        ...
        for (int i = 0; i < query_row; i++) {
            for (int j = 0; j < i + 1; j++) {
                double pourOneSide = (curr[j] - 1) / 2;
                if (pourOneSide > 0) {
                    next[j] += pourOneSide;
                    next[j + 1] += pourOneSide;
                }
            }
            curr = next;
            next = new double[100];
