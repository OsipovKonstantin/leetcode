...
//    medium #priority queue #two pointers
//    Time complexity is O(k+m)*logm), where k is number of people to hire, m in number of people from each side of
//     array costs. We install priority queue with mlogm operations and then traverse k elements with klogm ones
...
public class SolutionDay79Third {
    public static long totalCost(int[] costs, int k, int candidates) {
        int n = costs.length;
        PriorityQueue<Integer> pqFirst = new PriorityQueue<>();
        PriorityQueue<Integer> pqLast = new PriorityQueue<>();
        int start = 0;
        int end = n - 1;
        for (int i = 0; i < candidates; i++) {
            if (start <= end) pqLast.offer(costs[end--]);
            if (start <= end) pqFirst.offer(costs[start++]);
        }
        long ans = 0;
        while (k-- > 0 && (!pqFirst.isEmpty() || !pqLast.isEmpty())) {
            if (pqFirst.isEmpty() || (!pqLast.isEmpty() && pqFirst.peek() > pqLast.peek())) {
                ans += pqLast.poll();
                if (start <= end) pqLast.offer(costs[end--]);
            } else {
                ans += pqFirst.poll();
