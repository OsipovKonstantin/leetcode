...
public class SolutionDay23Fifth {
    public static List<Boolean> checkArithmeticSubarrays(int[] nums, int[] l, int[] r) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            res.add(checkArithmetic(sub));
        }
        return res;
    }

    public static boolean checkArithmetic(int[] sub) {
        Arrays.sort(sub);
        int increment = sub[1] - sub[0];
        for (int j = 2; j < sub.length; j++) {
            if (sub[j] - sub[j - 1] != increment) {
                return false;
            }
        }
        return true;
