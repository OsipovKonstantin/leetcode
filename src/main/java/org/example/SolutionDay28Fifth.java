...
//    medium. Counting arrival time frequency is 5-10 times faster than sorting arrival time
...
//    You can only eliminate 1 monster.
public class SolutionDay28Fifth {
    public static int eliminateMaximum(int[] dist, int[] speed) {
        int n = dist.length;
        int[] counter = new int[n];
        for (int i = 0; i < n; i++) {
            int time = dist[i] / speed[i] + (dist[i] % speed[i] == 0 ? 0 : 1);
            if (time >= n) {
                continue;
            }
            counter[time]++;
