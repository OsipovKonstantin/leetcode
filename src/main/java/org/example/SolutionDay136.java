...
//    medium #prefix sum #sliding window
...
public class SolutionDay136 {
    public static int equalSubstring(String s, String t, int maxCost) {
        int n = s.length();
        int[] prefixSum = new int[n + 1];
        for (int i = 1; i <= n; i++) {
            prefixSum[i] = Math.abs(s.charAt(i - 1) - t.charAt(i - 1)) + (i <= 1 ? 0 : prefixSum[i - 1]);
        }
        int ans = 0;
        int start = 0;
        for (int end = 1; end <= n; end++) {
            if (prefixSum[end] - prefixSum[start] <= maxCost) ans = Math.max(ans, end - start);
            else start++;
        }
        return ans;
    }
