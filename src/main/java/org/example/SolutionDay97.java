...
//    Time complexity is O(nlogn), where n is number of points
...
public class SolutionDay97 {
    public static int findMinArrowShots(int[][] points) {
        int n = points.length;
        Arrays.sort(points, (a, b) -> (a[0] > b[0] || (a[0] == b[0] && a[1] > b[1])) ? 1 : -1);
        int ans = 0;
        for (int i = 0; i < n; i++) {
            if (i > 0) {
                int[] interval = points[i - 1];
                while (i < n && points[i][0] <= interval[1]) {
